...
import { usersAnswers as previewUsersAnswers } from '../../services/previewData';
  ...
  const fetchUsersAnswers = async () => {
    try {
      ...
      const { data } = isPreviewMode
        ? await previewUsersAnswers(courseId)
        : await axios.get(
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg text-slate-700 font-medium">Loading student answers...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {usersData.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-12 text-center">
            ...
            <h3 className="mt-4 text-lg font-medium text-slate-900">No student answers yet</h3>
            <p className="mt-2 text-slate-500">Students haven't taken any quizzes for this course.</p>
          </div>
        ) : (
          <>
            {/* Leaderboard */}
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {topStudents.map((student, index) => (
                  <div key={student.user._id} className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-4 flex items-center">
                    ...
                    <div className="ml-4 flex-1">
                      ...
                      <p className="text-sm text-slate-600">Avg: {student.averageScore.toFixed(1)} pts</p>
                    </div>
                    <div className="text-2xl font-bold text-emerald-700">{student.averageScore.toFixed(1)}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6">
              <h2 className="text-xl font-bold text-slate-900 mb-4">All Students ({usersData.length})</h2>
              <div className="space-y-4">
                {usersData.map((student) => (
                  <div key={student.user._id} className="border border-slate-200 rounded-xl overflow-hidden">
                    {/* User header */}
                    <div
                      className="bg-slate-50 p-4 flex items-center justify-between cursor-pointer hover:bg-slate-100 transition"
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <p className="font-semibold text-slate-900">{student.user.name}</p>
                          <p className="text-sm text-slate-500">{student.answers.length} quizzes taken</p>
                        </div>
                      </div>
                      <div className="flex items-center">
                        <div className="mr-4 text-right">
                          <p className="text-sm text-slate-500">Average</p>
                          <p className="text-xl font-bold text-emerald-600">{student.averageScore.toFixed(1)}</p>
                        </div>
                        <svg className={`w-5 h-5 text-slate-400 transform transition-transform ${expandedUser === student.user._id ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {expandedUser === student.user._id && (
                      <div className="p-4 border-t border-slate-200 bg-white">
                        <h3 className="text-sm font-medium text-slate-700 mb-3">Quiz Attempts</h3>
                        <div className="space-y-3">
                          {student.answers.map((answer) => (
                            <div key={answer._id} className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                              <div className="flex justify-between items-start">
                                <div className="flex-1">
                                  <p className="text-sm font-medium text-slate-900">{answer.quizSnapshot.question}</p>
