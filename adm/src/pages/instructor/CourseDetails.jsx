...
const CourseDetails = () => {
  ...
  const addContent = async () => {
    ...
    try {
      ...
      // Free courses force all content to "free" — the per-content
      // "paid"/"free" availability selector is hidden for them, so always
      // send "free" (the backend also enforces this as a safety net).
      formData.append("availability", course?.isFree ? "free" : availability);
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl shadow p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => instructorLocked ? navigate("/payInstructor") : navigateToVideoSeries()}
            ...
            >
              ...
              {instructorLocked ? "Instructor Plan Required" : "Manage Videos"}
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow p-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <button
            onClick={() => instructorLocked ? navigate("/payInstructor") : setShowModal(true)}
            className={`px-5 py-2.5 rounded-lg transition-all flex items-center justify-center gap-2 w-full sm:w-auto ${
              instructorLocked
                ? "bg-slate-100 text-slate-500 hover:bg-slate-200"
          ...
          >
            ...
            {instructorLocked ? "Instructor Plan Required" : "Add Content"}
          </button>
      ...
      <div className="space-y-4">
        {displayedContents.length === 0 ? (
          <div className="bg-white rounded-xl shadow p-8 text-center">
            ...
            >
              {instructorLocked && <LockClosedIcon className="h-4 w-4" />}
              {instructorLocked ? "Instructor Plan Required" : "Add your first content"}
            </button>
          </div>
        ) : (
  ...
  <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl">
      ...
      <div className="space-y-4">
        ...
        {/* Hidden for free courses — a free course makes all its content
            available/free, so a per-content paid/free selector is not
            applicable and could create contradictory states. */}
        {!course?.isFree && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
